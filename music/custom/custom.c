...
void custom_init(lv_ui *ui)
{
    ...
    lv_obj_add_event(guider_ui.screen_img_album, album_gesture_event_cb, LV_EVENT_GESTURE, NULL);
    ...
    lv_obj_set_pos(spectrum_area, 80 * LV_HOR_RES / 480, 60 * LV_VER_RES / 272);
    lv_obj_set_size(spectrum_area, 320 * LV_HOR_RES / 480, 146 * LV_VER_RES / 272);
    lv_obj_move_background(spectrum_area);
    lv_obj_add_event(guider_ui.screen_player, spectrum_draw_event_cb, LV_EVENT_ALL, NULL);
}
...
void tracks_up(void)
{
	...
	lv_anim_set_values(&screen_player_anim_y, 0, -261 * LV_VER_RES / 272);
...
void tracks_down(void)
{
	...
	lv_anim_set_time(&screen_player_anim_y, 1000);
	lv_anim_set_exec_cb(&screen_player_anim_y, (lv_anim_exec_xcb_t)lv_obj_set_y);
	lv_anim_set_values(&screen_player_anim_y, -261 * LV_VER_RES / 272, 0);
	lv_anim_set_path_cb(&screen_player_anim_y, &lv_anim_path_linear);
	lv_anim_start(&screen_player_anim_y);
...
static lv_obj_t * album_img_create(lv_obj_t * parent)
{
	...
	lv_obj_set_pos(img, 187 * LV_HOR_RES / 480, 84 * LV_VER_RES / 272);
...
static void track_load(uint32_t id)
{
    ...
    if(next) {
        lv_anim_set_values(&a, 187 * LV_HOR_RES / 480, 0);
    } else {
        lv_anim_set_values(&a, 187 * LV_HOR_RES / 480, LV_HOR_RES - 105);
    }
...
void lv_demo_music_list_btn_check(uint32_t track_id, bool state)
{
    ...
    if(state) {
        ...
        lv_img_set_src(icon, &_btn_list_pause_alpha_42x42);
        lv_obj_scroll_to_view(btn, LV_ANIM_ON);
    }
    else {
        lv_obj_clear_state(btn, LV_STATE_PRESSED);
        lv_img_set_src(icon, &_btn_list_play_alpha_42x42);
    }
}
...
static void spectrum_draw_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        center.x = 187 * LV_HOR_RES / 480 + center_value / 2 + obj->coords.x1;
        center.y = 84 * LV_VER_RES / 272 + center_value / 2 + obj->coords.y1;
